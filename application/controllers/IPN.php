...
class IPN extends CI_Controller 
{
	...
	public function checkout2() 
	{
		...
		if($invoice->checkout2 == 0) {
			$hashSecretWord = $settings->checkout2_secret; //2Checkout Secret Word
			$hashSid = $settings->checkout2_accountno; //2Checkout account number
		} else {
			$hashSecretWord = $invoice->checkout2_secret;
			$hashSid = $settings->checkout2_accountno;
		}
		...
		$hashTotal = $invoice->total; //Sale total to validate against
		...
		if($key == $StringToHash) {
			...
			$this->session->set_flashdata("globalmsg", "The Invoice was not paid due to error!");
			redirect(site_url("invoices/view/" . $id ."/" . $hash));
		}
	}

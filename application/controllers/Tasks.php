		...
		if($proposal_id != 0) {
			...
			if (isset($data['taskid'])) {
				foreach($data['taskid'] as $i => $d){
					if($d != 0 && !empty(trim($data['name'][$i]))) {
						$tasks[$ctr]['proposal_service_task_id'] = $i;
			...
			if (isset($data['name'])) {
				foreach($data['name'] as $i => $d){
					if (!in_array($i, $data['taskid']) && !empty(trim($d))) {
						$newtasks[$ctr]['proposal_service_task_id'] = 0;
						$newtasks[$ctr]['name'] = $d;
			...
			if (isset($data['subtask'])) {
				foreach($data['subtask'] as $i => $data){
					...
					//insert new task
					if (!isset($data['new'])) {
						$subtasks[$ctr]['proposal_service_task_id'] = $i;
		   ...
		   // $this->dd($subtasksnew);
			...
			//echo "<pre>"; print_r($tasks); print_r($newtasks); print_r($subtasks); print_r($subtasksnew); die;
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////
			/*DATABASE ACTION*/
			...
			foreach($tasks as $task){
                ...
                $template_id = $this->task_model->edit_proposal_service_task($task['proposal_service_task_id'], $insert_data);
                ...
                if(!empty($subtask_holder)){
                    foreach($subtask_holder as $subtask){
                        ...
                        $this->task_model->edit_proposal_service_task($task['proposal_service_task_id'], $insert_subtask);
                    }
                }
			}
			...
			foreach($subtasks as $task){
                ...
                $this->task_model->edit_proposal_service_task($task['proposal_service_task_id'], $insert_data);
			}

            //Update service update status (Use to redirect the page according to service update status)
            $this->proposals_model->update_proposal_service_update_status($serviceid);
		} else {
			//A. update existing task
			if (isset($data['taskid'])) {
				foreach($data['taskid'] as $i => $d){

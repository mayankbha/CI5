...
//        $this->check_requirements();
    ...
    public function update ($id, $proposal_id=0)
    {
		...
		if($proposal_id != 0) {
			$proposal_service = $this->services_model->get_proposal_service($this->user->info->ID, $id);
		...
		if($proposal_id != 0) {
			if(!empty($proposal_service)) {
				...
				//$service_list['ID'] = $service->proposal_service_id;
				$service_list['proposal_service_id'] = $service->proposal_service_id;
				$service_list['ID'] = $service->service_id;
				$service_list['name'] = $service->name;
				$service_list['price'] = $service->price;
				$service_list['description'] = $service->description;
				$service_list['notes'] = $service->notes;
				$service_list['recurring'] = $service->recurring;
				$service_list['interval'] = $service->interval;
				$service_list['interval_label'] = $service->interval_label;
				$service_list['set_price'] = $service->set_price;
				$service_list['hours'] = $service->hours;
				$service_list['filename'] = $service->filename;
				$service_list['filetype'] = $service->filetype;
				$service_list['encrypt'] = $service->encrypt;
				//$service_list['username'] = $service->username;
				//$service_list['avatar'] = $service->avatar;
				//$service_list['online_timestamp'] = $service->online_timestamp;
				//$service_list['assigned_email'] = $service->assigned_email;
				//$service_list['assigned_email_notification'] = $service->assigned_email_notification;
			} else {
				$id = $service->ID;
		...
		//echo "<pre>"; print_r($service); print_r($service_list); die;
        $billing_roles = $this->team_model->get_billing_roles();
		...
		if($proposal_id != 0) {
			//$proposal_service_tasks[$id] = $this->services_model->get_proposal_task_service($id);
			$proposal_service_tasks[$id] = $this->services_model->get_proposal_service_tasks($id, $proposal_id);

			$tasks[$id] = $this->services_model->get_task_service($id);

			//echo "<pre>"; print_r($tasks);

			foreach($tasks[$id] as $key2 => $task2) {
				if(isset($proposal_service_tasks[$id][$key2]['service_id'])) {
					//echo 'in if';

					$tasks_list[$id][$key2]['proposal_service_task_id'] = $proposal_service_tasks[$id][$key2]['proposal_service_task_id'];
					$tasks_list[$id][$key2]['ID'] = $proposal_service_tasks[$id][$key2]['proposal_service_task_id'];
					//$tasks_list[$id][$key2]['ID'] = $proposal_service_tasks[$id][$key2]['project_task_template_id'];
					$tasks_list[$id][$key2]['service_id'] = $proposal_service_tasks[$id][$key2]['proposal_service_id'];
					$tasks_list[$id][$key2]['is_subtask'] = $proposal_service_tasks[$id][$key2]['is_subtask'];
					$tasks_list[$id][$key2]['parent_id'] = $proposal_service_tasks[$id][$key2]['parent_id'];
					$tasks_list[$id][$key2]['name'] = $proposal_service_tasks[$id][$key2]['name'];
					$tasks_list[$id][$key2]['description'] = $proposal_service_tasks[$id][$key2]['description'];
					$tasks_list[$id][$key2]['price'] = $proposal_service_tasks[$id][$key2]['price'];
					$tasks_list[$id][$key2]['billing_role'] = $proposal_service_tasks[$id][$key2]['billing_role'];
					$tasks_list[$id][$key2]['recurring'] = $proposal_service_tasks[$id][$key2]['recurring'];
					$tasks_list[$id][$key2]['due_count'] = $proposal_service_tasks[$id][$key2]['due_count'];
					$tasks_list[$id][$key2]['hours'] = $proposal_service_tasks[$id][$key2]['hours'];
					$tasks_list[$id][$key2]['filename'] = $proposal_service_tasks[$id][$key2]['filename'];
					$tasks_list[$id][$key2]['filetype'] = $proposal_service_tasks[$id][$key2]['filetype'];
					$tasks_list[$id][$key2]['encrypt'] = $proposal_service_tasks[$id][$key2]['encrypt'];
					$tasks_list[$id][$key2]['start_date'] = $proposal_service_tasks[$id][$key2]['start_date'];
					$tasks_list[$id][$key2]['due_date'] = $proposal_service_tasks[$id][$key2]['due_date'];
					$tasks_list[$id][$key2]['status'] = $proposal_service_tasks[$id][$key2]['status'];
					$tasks_list[$id][$key2]['userid'] = $proposal_service_tasks[$id][$key2]['userid'];
					$tasks_list[$id][$key2]['complete'] = $proposal_service_tasks[$id][$key2]['complete'];
					$tasks_list[$id][$key2]['complete_sync'] = $proposal_service_tasks[$id][$key2]['complete_sync'];
					$tasks_list[$id][$key2]['archived'] = $proposal_service_tasks[$id][$key2]['archived'];
				} else {
					//echo 'in else';

					$tasks_list[$id][$key2]['ID'] = $task2['ID'];
					$tasks_list[$id][$key2]['service_id'] = $task2['service_id'];
					$tasks_list[$id][$key2]['is_subtask'] = $task2['is_subtask'];
					$tasks_list[$id][$key2]['parent_id'] = $task2['parent_id'];
					$tasks_list[$id][$key2]['name'] = $task2['name'];
					$tasks_list[$id][$key2]['description'] = $task2['description'];
					$tasks_list[$id][$key2]['price'] = $task2['price'];
					$tasks_list[$id][$key2]['billing_role'] = $task2['billing_role'];
					$tasks_list[$id][$key2]['recurring'] = $task2['recurring'];
					$tasks_list[$id][$key2]['due_count'] = $task2['due_count'];
					$tasks_list[$id][$key2]['hours'] = $task2['hours'];
					$tasks_list[$id][$key2]['filename'] = $task2['filename'];
					$tasks_list[$id][$key2]['filetype'] = $task2['filetype'];
					$tasks_list[$id][$key2]['encrypt'] = $task2['encrypt'];
					$tasks_list[$id][$key2]['start_date'] = $task2['start_date'];
					$tasks_list[$id][$key2]['due_date'] = $task2['due_date'];
					$tasks_list[$id][$key2]['status'] = $task2['status'];
					$tasks_list[$id][$key2]['userid'] = $task2['userid'];
					$tasks_list[$id][$key2]['complete'] = $task2['complete'];
					$tasks_list[$id][$key2]['complete_sync'] = $task2['complete_sync'];
					$tasks_list[$id][$key2]['archived'] = $task2['archived'];
				}
			}
		} else {
			$tasks[$id] = $this->services_model->get_task_service($id);

			foreach($tasks[$id] as $key => $task) {
				$tasks_list[$id][$key]['ID'] = $task['ID'];
				$tasks_list[$id][$key]['service_id'] = $task['service_id'];
				$tasks_list[$id][$key]['is_subtask'] = $task['is_subtask'];
				$tasks_list[$id][$key]['parent_id'] = $task['parent_id'];
				$tasks_list[$id][$key]['name'] = $task['name'];
				$tasks_list[$id][$key]['description'] = $task['description'];
				$tasks_list[$id][$key]['price'] = $task['price'];
				$tasks_list[$id][$key]['billing_role'] = $task['billing_role'];
				$tasks_list[$id][$key]['recurring'] = $task['recurring'];
				$tasks_list[$id][$key]['due_count'] = $task['due_count'];
				$tasks_list[$id][$key]['hours'] = $task['hours'];
				$tasks_list[$id][$key]['filename'] = $task['filename'];
				$tasks_list[$id][$key]['filetype'] = $task['filetype'];
				$tasks_list[$id][$key]['encrypt'] = $task['encrypt'];
				$tasks_list[$id][$key]['start_date'] = $task['start_date'];
				$tasks_list[$id][$key]['due_date'] = $task['due_date'];
				$tasks_list[$id][$key]['status'] = $task['status'];
				$tasks_list[$id][$key]['userid'] = $task['userid'];
				$tasks_list[$id][$key]['complete'] = $task['complete'];
				$tasks_list[$id][$key]['complete_sync'] = $task['complete_sync'];
				$tasks_list[$id][$key]['archived'] = $task['archived'];
			}
		}

		//echo "<pre>"; print_r($tasks); print_r($tasks_list); die;
		//$this->dd($services);
		//$this->dd($tasks[$id]);

        $this->template->loadContent("services/update.php", array(
        		"service" => (object) $service_list,
        		"tasks" => $tasks_list,
                "billing_roles" => $billing_roles,
				"proposal_id" => $proposal_id
				)
    ...
    public function edit_pro ($service_id, $proposal_id=0)
    {
		...
		if($proposal_id != 0) {
			...
			//echo "<pre>"; print_r($proposal_service); die;

			if(!empty($proposal_service)) {
				$service_id = $proposal_service->proposal_service_id;

				$this->services_model->edit_proposal_service($proposal_service->proposal_service_id, array(
					'name' => $_POST['service_name'],
					'description' => $_POST['service_description'],
					'notes' => $_POST['service_notes'],
				...
				$service_id = $this->services_model->insert_proposal_service(array(

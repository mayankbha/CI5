...
class Proposals extends CI_Controller 
{
	...
	public function get_templates() {
		...
		$this->datatables->set_total_rows($this->proposals_model->get_user_template_total($user_id));

		$templates = $this->proposals_model->get_user_templates($user_id, $this->datatables);

		foreach($templates as $template) {
			if($this->common->has_permissions(array("admin"), $this->user))
				$options = '<a target="_blank" href="'.site_url("proposals/view/".$template->template_id."/2").'" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-list-alt"></span></a> <a target="_blank" href="'.site_url("proposals/edit/" . $template->template_id).'" class="btn btn-warning btn-xs" title="'.lang("ctn_55").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("proposals/delete_template/".$template->template_id).'" class="btn btn-danger btn-xs" onclick="return confirm(\''.lang("ctn_508").'\')" title="'.lang("ctn_57").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>';
			else
	...
	public function add() {
		...
		if($this->input->post()) {
			...
			$this->proposals_model->save_proposal_as_template(array("userid" => 0, 'image' => '', 'content' => $content));
		}
	...
	public function edit_template($template_id = 0) {
		...
		if($template_id != 0) {
			...
			if($this->input->post()) {
				...
				$this->proposals_model->eidt_template($template_id, array('image' => '', 'content' => $content));
			}
		}

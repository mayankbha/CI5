...
class Projects extends CI_Controller 
{
	...
	public function view($id, $page=0) 
	{
        ...
        $tasks = $this->task_model->get_all_project_tasks($id);
	...
	public function add_project_by_service(){

		$params = array(
			'return_project_id' => 1, //this just tells the function to return the project id
			'name' => $_POST['name'],
			'description' => $_POST['name'] . ' created on '.date('d F Y'),
			'catid' => 6, //in progress
            'date_start' => date('Y-m-d', strtotime($_POST['date_start']))
		);
		$project_id = $this->add_project($params);
		redirect(site_url("tasks/add_process?project_id=".$project_id."&".urldecode(http_build_query($_POST))));
	}

	public function add_project($params = null)
	{
		if(!$this->common->has_permissions(array("admin", "project_admin", 
			"project_worker"), 
			$this->user)) {
		...
		$projectid = $this->projects_model->add_project(array(
			...
			"timestamp" => time(),
			"image" => $image,
			"calendar_id" => isset($calendar_id) ? $calendar_id : '',
			"calendar_color" => isset($calendar_color) ? $calendar_color: '',
			"complete" => $complete,
		...
		if(isset($params['return_project_id'])){
			return $projectid;
		}

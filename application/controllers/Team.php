...
class Team extends CI_Controller 
{
    ...
    public function billing_roles()
    {
        ...
        $this->template->loadData("activeLink",
            array("team" => array("billing_roles" => 1)));

        $roles = $this->team_model->get_billing_roles();
        $this->template->loadContent("roles/roles.php", array(
                "roles" => $roles
            )
        );
    }

    public function get_billing_role ()
    {
        $id = $_GET['id'];

        $roles = $this->team_model->get_billing_role($id);
    ...
    public function add_billing_role()
    {
        if(!$this->common->has_permissions(array("admin", "project_admin",
            "team_manage"),
            $this->user)) {
        ...
        $name = $this->common->nohtml($this->input->post("role_name"));
        $price = intval($this->input->post("role_price"));

        if(empty($name)) $this->template->error(lang("error_195"));

        $this->team_model->add_billing_role(array(
                "name" => $name,
                "price" => $price,
        ...
        $this->user_model->add_user_log(array(
                ...
                "IP" => $_SERVER['REMOTE_ADDR'],
                "projectid" => 0,
                "url" => "team/billing_roles"
            )
        );

        $this->session->set_flashdata("globalmsg",
            lang("success_95"));
        redirect(site_url("team/billing_roles"));
    }

    public function edit_billing_role($id)
    {
        if(!$this->common->has_permissions(array("admin", "project_admin",
            "team_manage"),
            $this->user)) {
        ...
        $this->template->loadData("activeLink",
            array("team" => array("billing_roles" => 1)));
        $id = intval($id);
        $role = $this->team_model->get_billing_role($id);
        if($role->num_rows() == 0) $this->template->error(lang("error_191"));

        $this->template->loadContent("roles/edit_billing_role.php", array(
                "role" => $role->row()
            )
        );
    }

    public function edit_billing_role_pro($id)
    {
        if(!$this->common->has_permissions(array("admin", "project_admin",
            "team_manage"),
            $this->user)) {
        ...
        $this->team_model->update_billing_role($id, array(
                "name" => $name,
                "price" => $price,
                "user_id" => $this->user->info->ID
            )
    ...
    public function delete_billing_role($id, $hash)
    {
        if(!$this->common->has_permissions(array("admin", "project_admin",
            "team_manage"),
            $this->user)) {
        ...
        // Delete
        $this->team_model->delete_billing_role($id);
        $this->session->set_flashdata("globalmsg",
            lang("success_97"));
        redirect(site_url("team/billing_roles"));
    }

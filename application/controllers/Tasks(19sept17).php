    ...
    public function edit_service_task($serviceid, $proposal_id=0){
		...
		if($proposal_id != 0) {
		   ...
		   // $this->dd($subtasksnew);
			...
			//echo "<pre>"; print_r($tasks); print_r($subtasksnew);
			///////////////////////////////////////////////////////////////////////////////////////////////////////////////
			/*DATABASE ACTION*/
			...
			foreach($tasks as $task){
				...
				$proposal_service_task = $this->services_model->check_proposal_service_task_exists($serviceid, $task['id']);

				//$proposal_service_task = $this->services_model->get_proposal_service_task($proposal_id, $task['id']);

				if(!empty($proposal_service_task)) {
					$template_id = $this->task_model->edit_proposal_service_task($proposal_service_task->proposal_service_task_id, $insert_data);

					/*A. subtask*/
					if(!empty($subtask_holder)){
						foreach($subtask_holder as $subtask){
							$insert_subtask = $subtask;
							$insert_subtask['proposal_service_id'] = $serviceid;
							$insert_subtask['is_subtask'] = 1;
							$insert_subtask['parent_id'] = $template_id;

							//$this->task_model->edit_proposal_service_task($insert_subtask);
						}
					}
				} else {
					$insert_data['proposal_service_id'] = $serviceid;
			...
			foreach($subtasksnew as $task){
				$insert_data = $task;
				$insert_data['proposal_service_id'] = $serviceid;
				$insert_data['project_task_template_id'] = $key;
...
//echo "<pre>"; print_r($subtasks);
			...
			foreach($subtasks as $task){
				...
				//echo "<pre>"; print_r($subtasks); die;
				$proposal_service_subtask = $this->services_model->check_proposal_service_subtask_exists($serviceid, $task['id']);

				//echo "<pre>"; print_r($proposal_service_subtask); die;
				if(!empty($proposal_service_subtask)) {
					$insert_data['parent_id'] = $proposal_service_subtask->proposal_service_task_id;

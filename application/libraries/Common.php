    ...
    */
    public function getDatesFromRange($start, $end, $format = 'Y-m-d', $format_db = "d-n-Y") 
    {
        ...
        foreach($period as $date) { 
            $array[] = array("display" => $date->format($format), "db" => $date->format($format_db), "timestamp" => $date->getTimestamp(), "obj" => $date); 
        }
	...
	public function get_public_templates() {
		...
		$public_templates = $CI->proposals_model->get_public_templates();

		if(!empty($public_templates)) {
			//echo "<pre>"; print_r($public_templates);
	...
	public function get_user_templates($user_id) {
		...
		$user_templates = $CI->proposals_model->get_user_templates($user_id);

		if(!empty($user_templates)) {
			//echo "<pre>"; print_r($user_templates);

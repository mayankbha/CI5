...
class Task_Model extends CI_Model 
{
	...
	public function get_all_project_tasks($projectid) 
	{
		return $this->db
			...
			->select("project_tasks.name, project_tasks.ID, 
				...
				project_tasks.due_date, project_tasks.start_date, 
				project_tasks.description, project_tasks.complete,
                project_tasks.price, project_tasks.hours,
    ...
    public function get_task_template($taskid){
        return $this->db->where("ID", $taskid)->get("project_task_template");
    }

	public function insert_task_template($params){
		$this->db->insert("project_task_template", $params);
		return $this->db->insert_id();
	}

    public function edit_task_template($id, $data){
        $this->db->where("ID", $id)->update("project_task_template", $data);
    }
	
	public function insert_proposal_service_task($params){
		$this->db->insert("proposal_service_tasks", $params);
		return $this->db->insert_id();
	}

    public function edit_proposal_service_task($id, $data){
        $this->db->where("proposal_service_task_id", $id)->update("proposal_service_tasks", $data);
    }
    
    public function get_project_task_template_data($task_template_id) {
        return $this->db->where("ID", $task_template_id)->get("project_task_template")->row();
    }
}

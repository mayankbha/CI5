	...
	<script type="text/javascript">
		...
		function send_proposal(proposal_id, proposal_type, btn_id) {
			...
			$.ajax({
				...
				data: {'proposal_id' : proposal_id, 'proposal_type' : proposal_type, 'emails' : emails, 'discount' : discount, 'proposal_validity' : proposal_validity, 'price' : price},
				cache: false,
				success: function(res) {
					//alert('success :: '+res);
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data==1){
                        ...
                        updateServicesStatus(service_id);
                    }else{
                        setTimeout(function(){
        ...
        function updateServicesStatus(service_id) {
            $.ajax({
                type: "POST",
                url: global_base_url+"proposals/updateproposalServicesStatus",
                cache: false,
                data: {'service_id' : service_id, 'proposal_id' : <?php echo $proposal_id; ?>, 'status' : 0,'updateServicesStatus':true},
            ...
            $.ajax({
				...
				success: function(data) {
                    ...
                    $.each(json, function (key, data) {
                        ...
                        services += "<div class='row'><div class='count_price_div'><span style='display: none;'>"+data.service_id+"</span> <span style='display: none;'>"+data.name+"</span> <span style='display: none;'>"+data.description+"</span> <span style='display: none;'>"+recurring+"</span> <span style='display: none;'>"+data.price+"</span></div><div class='col-s-12 occeurancediv'><span class='occeurance'><b>"+data.name+" ("+data.interval+ " "+data.interval_label+")</b></span></div><div class='col-s-12 durationdiv'><span >"+data.description+"</span></div></div><div class='col-s-12 attachmentdiv'></br><span class='attachment'><div class='attachmentvalue'><a class='update_service_link_common update_service_link-"+data.service_id+"' id='"+data.service_id+"' href='javascript: void();' onclick='showEditProposalServiceModal("+data.service_id+", "+data.proposal_service_id+", "+proposal_id+");'>Edit</a></div></span></div>";
                    });

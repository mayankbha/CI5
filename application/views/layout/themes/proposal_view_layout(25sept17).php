...
<html lang="en">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="form-horizontal" role="form" name="send_proposal_form" id="send_proposal_form" action="">
                            <input type="hidden" class="form-control" id="price" name="price" value="<?php echo $proposal->price; ?>" />
	...
	<?php if($proposal_type == 1) { ?>
		<script type="text/javascript">
			...
			function send_proposal(proposal_id, proposal_type) {
				$.ajax({
					type: "POST",
					url: global_base_url+"proposals/send_proposal",
					data: {'proposal_id' : proposal_id, 'proposal_type' : proposal_type},
					cache: false,
					success: function(res) {
						//alert('success :: '+res);
			...
			function getContent(proposal_id, status) {
				...
				$.ajax({
					...
					data: {'proposal_id' : proposal_id, 'content' : content, 'status' : status},
					cache: false,
					success: function(res) {
						console.log('success');

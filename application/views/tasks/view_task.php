...
<?php foreach($taskreference->result() as $r) : ?>
    <?php if (!empty($r->filename)) : ?>
        <?php echo "<a href='".base_url()."uploads/tasks/".$r->encrypt."' download>".$r->filename."</a>"; ?>
        <?php $attachment_count +=1; ?>
    <?php endif; ?>
<?php endforeach; ?>
...
<?php foreach($objectives->result() as $r) : ?>
	<div class="row">
      ...
      <?php endif; ?>
            ...
            <br/>
            <div class="task-objective-files"><b><?php echo lang("ctn_345") ?></b></div>
                <p class="small-text">
                    <?php $CI =& get_instance();?>
                    <?php if (isset($objective_reference[$r->ID]) && isset($objective_reference[$r->ID][0])): ?>
                        <?php if (!empty($objective_reference[$r->ID][0]->filename && file_exists(FCPATH."/uploads/tasks/".$objective_reference[$r->ID][0]->encrypt))): ?>
                        <?php echo "<a href='".base_url() . $CI->settings->info->upload_path_relative."/tasks/".$objective_reference[$r->ID][0]->encrypt."' download>".$objective_reference[$r->ID][0]->filename."</a>" ?>
                        <?php endif; ?>
                    <?php endif; ?>
                </p>
            <br/>
